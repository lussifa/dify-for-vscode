const { createPresentation, updatePresentation, inspectPresentation } = require('./officePpt');
...
const pptSlide = obj({
  type: enumStr(['title','section','bullets','two_column','table','chart','quote','blank']),
  title: str(), subtitle: str(), bullets: arr(str(), { maxItems: 20 }),
  left_title: str(), left_bullets: arr(str(), { maxItems: 20 }),
  right_title: str(), right_bullets: arr(str(), { maxItems: 20 }),
  quote: str(), attribution: str(), table: pptTable, chart: pptChart, notes: str()
}, ['type']);

const wordTable = obj({ headers: arr(str(), { maxItems: 30 }), rows: arr(arr(str(), { maxItems: 30 }), { maxItems: 200 }) });
const wordBlock = obj({
  type: enumStr(['heading','paragraph','bullets','numbered','table','quote','page_break']),
  text: str(), level: int(1, 6), items: arr(str(), { maxItems: 100 }), table: wordTable
}, ['type']);

const OFFICE_TOOLS = [
  fn('ppt_create', 'Create a polished PowerPoint .pptx from a declarative slide specification. Use high-level slide layouts instead of manually positioning shapes. The file is written inside the current workspace and an editable Dify sidecar is saved for later ppt_update calls.', obj({
    output_path: str('Workspace-relative .pptx output path.'), title: str(), subject: str(), author: str(), company: str(),
    theme: enumStr(['corporate-light','corporate-dark','minimal','training']), font_family: str('Presentation font family.'),
    slides: arr(pptSlide, { minItems: 1, maxItems: 100 })
  }, ['output_path','slides'])),
  fn('ppt_update', 'Update a PowerPoint previously generated by ppt_create by rebuilding it from its saved declarative sidecar. Supports replacing/appending/deleting a slide or changing the presentation title.', obj({
    path: str('Workspace-relative .pptx path.'), action: enumStr(['replace_slide','append_slide','delete_slide','set_title']),
    slide_index: int(1, 100), slide: pptSlide, title: str()
  }, ['path','action'])),
  fn('ppt_inspect', 'Inspect a PowerPoint file. For presentations generated by ppt_create, returns structured slide metadata and optionally the full editable spec.', obj({ path: str(), include_spec: bool() }, ['path'])),
  ...
  fn('excel_create', 'Create a styled Excel .xlsx workbook with one or more sheets, headers, rows, freeze panes, filters, widths, number formats and formulas. Best for reports, manifests, summaries and tabular office automation.', obj({
    ...
    sheets: arr(obj({
      name: str(), headers: arr(str(), { maxItems: 200 }), rows: arr(rowSchema, { maxItems: 10000 }),
      freeze_header: bool(), auto_filter: bool(), style: enumStr(['corporate','dark','training']),
      column_widths: arr(num(), { maxItems: 200 }), number_formats: obj({}, [], true)
    }, ['name']), { minItems: 1, maxItems: 50 })
  }, ['output_path','sheets'])),
  ...
  fn('excel_format_range', 'Apply common formatting to a rectangular A1 range in an existing Excel worksheet.', obj({
    path: str(), sheet: str(), range: str('A1 range such as A1:F20.'),
    style: obj({ bold: bool(), italic: bool(), font_size: num(), font_color: str('Hex RGB.'), fill_color: str('Hex RGB.'), number_format: str(), horizontal: enumStr(['left','center','right','fill','justify','centerContinuous','distributed']), vertical: enumStr(['top','middle','bottom','distributed','justify']), wrap_text: bool(), border: bool() })
  }, ['path','range','style'])),

  fn('word_create', 'Create a professional Word .docx document from declarative content blocks such as headings, paragraphs, bullets, tables and quotes. An editable Dify sidecar is saved for reliable later updates.', obj({
    output_path: str('Workspace-relative .docx output path.'), title: str(), author: str(), subject: str(), company: str(), font_family: str(),
    blocks: arr(wordBlock, { minItems: 1, maxItems: 500 })
  }, ['output_path','blocks'])),
  fn('word_update', 'Update a Word document previously generated by word_create by rebuilding it from its saved declarative sidecar.', obj({
    path: str(), action: enumStr(['replace_block','append_block','delete_block','set_title']), block_index: int(1, 500), block: wordBlock, title: str()
  }, ['path','action'])),
...
const OFFICE_MUTATING = new Set([
  'ppt_create','ppt_update','excel_create','excel_write_range','excel_append_rows','excel_format_range','word_create','word_update','office_render_pdf'
]);
