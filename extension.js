...
class DifyChatViewProvider {
  ...
  async toggleYolo() {
    ...
    if (next) {
      const choice = await vscode.window.showWarningMessage('Enable YOLO mode?', { modal: true, detail: 'YOLO automatically approves file mutations, patches and shell commands requested by the Dify model.' }, 'Enable YOLO');
  ...
  async sendUserMessage(text) {
    ...
    if (!apiKey) { this.post({ type: 'error', message: 'Dify API key is not configured.' }); await this.configure(); return; }
    this.running = true;
  ...
  async agentLoop(firstQuery, apiKey) {
    const maxSteps = this.config().get('maxAgentSteps', 30);
