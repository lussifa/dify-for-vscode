...
class DifyChatViewProvider {
  ...
  async toggleYolo() {
    ...
    if (next) {
      const choice = await vscode.window.showWarningMessage('Enable YOLO mode?', { modal: true, detail: 'YOLO automatically approves mutating file tools, browser actions, shell commands, Office writes and potentially mutating MCP tools requested by the Dify model.' }, 'Enable YOLO');
